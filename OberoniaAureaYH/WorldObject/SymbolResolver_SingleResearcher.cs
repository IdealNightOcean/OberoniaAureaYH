...
public class SymbolResolver_SingleResearcher : SymbolResolver
{
    ...
    public override void Resolve(ResolveParams rp)
    {
        ...
        if (rp.singlePawnToSpawn is null)
        {
            ...
            {
                PawnKindDef pawnKindDef = rp.singlePawnKindDef ?? DefDatabase<PawnKindDef>.AllDefsListForReading.Where((PawnKindDef x) => x.defaultFactionType is null || !x.defaultFactionType.isPlayer).RandomElement();
                Faction result = rp.faction;
                if (result is null && pawnKindDef.RaceProps.Humanlike)
                {
                    if (pawnKindDef.defaultFactionType is not null)
                    {
